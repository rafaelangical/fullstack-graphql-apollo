...
function relayApplyFragmentArgumentTransform(context) {
  ...
  var nextContext = require("./GraphQLIRTransformer").transform(context, {
    Root: function Root(node) {
      var scope = require("./RelayCompilerScope").getRootScope(node.argumentDefinitions);
...
function transformCondition(context, fragments, scope, node, errorContext) {
  ...
  if (!(condition.kind === 'Literal' || condition.kind === 'Variable')) {
    ...
    throw require("./RelayCompilerError").createNonRecoverableUserError('A non-scalar value was applied to an @include or @skip directive, ' + 'the `if` argument value must be a ' + 'variable or a literal Boolean.', [condition.loc]);
  }
...
function transformSelections(context, fragments, scope, selections, errorContext) {
  ...
  selections.forEach(function (selection) {
    ...
    if (selection.kind === 'InlineFragment' || selection.kind === 'MatchBranch') {
      ...
      throw require("./RelayCompilerError").createCompilerError('RelayApplyFragmentArgumentTransform: Expected to be applied before processing @defer/@stream.', [selection.loc]);
    } else {
      selection;
      throw require("./RelayCompilerError").createCompilerError("RelayApplyFragmentArgumentTransform: Unsupported kind '".concat(selection.kind, "'."), [selection.loc]);
    }
...
function transformValue(scope, value, errorContext) {
  if (value.kind === 'Variable') {
    ...
    if (scopeValue == null) {
      ...
      // a single document could break invariants and/or cause
      // additional spurious errors.
      throw require("./RelayCompilerError").createNonRecoverableUserError("Variable '$".concat(value.variableName, "' is not in scope."), [value.loc]);
    }
  ...
  var printedArgs = JSON.stringify(sortedArgs.map(function (arg) {
    ...
    if (arg.value.kind === 'Variable') {
      ...
      if (value == null) {
        ...
        // a single document could break invariants and/or cause
        // additional spurious errors.
        throw require("./RelayCompilerError").createNonRecoverableUserError("Variable '$".concat(arg.value.variableName, "' is not in scope."), [arg.value.loc]);
      }
    } else {
      value = arg.value;
    }
    ...
    return {
      name: arg.name,
      value: require("./getIdentifierForArgumentValue")(value)
    };
  }));
